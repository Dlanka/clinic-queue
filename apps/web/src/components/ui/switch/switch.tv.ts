...
export const switchStyles = tv({
  slots: {
    ...
    track:
      "relative ring ring-transparent h-6 w-11 rounded-full transition-all duration-200 outline-none focus:ring-2",
    thumb:
      "absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-neutral-100 shadow transition-transform duration-200 flex items-center justify-center text-xs font-bold text-neutral-95",
  ...
  variants: {
    checked: {
      true: {
        ...
        thumb: "translate-x-5 text-neutral-30"
      },
      false: {
        track: "bg-neutral-40 ring-primary/30",
        thumb: "translate-x-0 bg-neutral-70 text-neutral-30"
      }
    },
    intent: {
  ...
  compoundVariants: [
    ...
    },
    {
      ...
      intent: "danger",
      class: {
        track: "bg-danger shadow-[0_0_12px_color-mix(in_srgb,var(--color-danger)_30%,transparent)]"
      }
    },
    {
      checked: true,
